...
class RobotAgent {
    ...
    fun act(obs: Observation, robot: Robot, actionMap: ActionMap): List<RobotAction>? {
        ...
        if (actionQueueCooldown > 0 || robot.power < robot.actionQueueCost() || obs.me.factories.none()) {
            return null
        }
        ...
        if (robot.cargo.ice == 0 && obs.board.ice[robot.tile] && obs.factories[robot.tile] == null) {
        ...
        val target = if (robot.cargo.ice == 0) {
            obs.board.ice.tiles()
                .filter { it.value && obs.factories[it.tile] == null }
