...
data class Observation(
    ...
    val factories = NullableGrid<Factory>(config.mapSize).apply {
        for (team in teams) {
    ...
    val robots = NullableGrid<Robot>(config.mapSize).apply {
        for (team in teams) {
    ...
    companion object {
        fun parse(raw: JSONObject, previous: Observation?, config: Config): Observation {
            ...
            val board = if (previous == null) {
                val rubble = Grid.parseIntGrid(rawBoard.getJSONArray("rubble"))

                val ore = Grid(rubble.size) { false }
                Grid.parseIntGrid(rawBoard.getJSONArray("ore")).tiles().forEach {
                    ore[it.tile.y, it.tile.x] = it.value > 0
                }

                val ice = Grid(rubble.size) { false }
                Grid.parseIntGrid(rawBoard.getJSONArray("ice")).tiles().forEach {
                    ice[it.tile.y, it.tile.x] = it.value > 0
                }

                val lichen = Grid.parseIntGrid(rawBoard.getJSONArray("lichen"))
                val strains = Grid.parseIntGrid(rawBoard.getJSONArray("lichen_strains"))
            ...
            for (playerId in rawTeams.keys()) {
                ...
                val factories = NullableGrid<Factory>(config.mapSize)
                ...
                val robots = NullableGrid<Robot>(config.mapSize)
